...
1.3. You MUST NOT add a required runtime dependency to the **launcher**. `scc`, `install.sh`, and `entrypoint.sh` run on the host/container with only: `bash`/`sh`, `docker`, and coreutils. No node, python, jq, or other interpreter may be *required* to launch. (Tools used *inside* the image are fine, and tools used by the *firewall* (`iptables`, `ipset`, `dig`, `jq`, `curl`) are already baked into the image and stay there.)
...
2.3. Subcommands MUST be additive. Each new subcommand is its own unit and MUST NOT change the behavior of existing ones. The reserved names are `yolo`, `shell`, `login`, `update`, `self-update`, `rebuild`, `build`, `profiles`, `init`, `trust`, `uninstall`, `version`, `help` (plus `--version`/`-V`). Everything else passes straight to `claude`. Preserve that passthrough. Reserving a new name changes passthrough, so it requires updating this list, `CLAUDE.md`, the README, and `help` in the same change (§7.2).
...
3.2. `--security-opt no-new-privileges:true`, `--pids-limit`, and `--init` MUST remain on every run. There MUST be no `sudo` in the image. The agent MUST NOT run as uid 0 in the container (running scc as host root would remap to it); the launcher (`scc_guard_not_root`) and the entrypoint both refuse unless `SCC_ALLOW_ROOT=1` is set.
3.3. The workspace mount MUST remain **only** the current directory (`-v "$PWD:$PWD"`), plus `~/.gitconfig` read-only and the home volume. You MUST NOT mount `$HOME`, SSH keys, host credentials, or arbitrary host paths by default. `guard_workdir` (refusing `$HOME` and `/`) MUST stay. There are **no host-state mounts on by default.** Clipboard forwarding for in-chat image paste is opt-in like every other integration (`--clipboard` for the run, or `clipboard = on`), never on by default, and off under `--hardened`. Any new default mount needs its own justification.
...
3.6. `scc yolo` (skip-permissions) MUST keep the firewall **on by default**. An agent that skips prompts does not also get open egress. `--hardened` is an **absolute floor**: it forces the firewall on even over an explicit `firewall=off`/`SCC_FIREWALL=0` (the override MUST be announced, not silent), because it is the user's own max-lockdown request. A trusted project's `firewall = on` **tightens over config/default only**: it MUST NOT be able to loosen the sandbox, but the user's explicit `SCC_FIREWALL` still wins (the trust model protects the user from the project, not the reverse).
...
6.1. This project is independent and unofficial. The disclaimer (*not affiliated with Anthropic and does not own, control, or bundle Claude Code, but installs Anthropic's official CLI at runtime*) MUST remain present and visible in the README, the Dockerfile, and the `scc` launcher header. You MUST NOT remove or soften it.
...
7.1. Docs MUST NOT overstate the sandbox. Keep the framing that this is a container, not a VM, that the kernel is shared, and that `yolo` is bounded, not neutralized. When you add a boundary, document its limits in the same breath.
